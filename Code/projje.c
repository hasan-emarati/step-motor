...
int i ;
int ertefa=0;
int bala = 1;    // baraye check kardan inke paye bimar bala hast ya nist
int play = 0;
int stepp = 0;
...
while (1)
 {
               sprintf(lcd_round,"%d",stepp);
               lcd_gotoxy(0,0);
               lcd_puts("Round:");
               if(stepp<10)
               lcd_puts("0");                                                                                                    //namayesh dor motor
               lcd_puts(lcd_round);
               /////////////////////////////////////////////////////////////////



                if(play==0 ){                                                      // dastgah dar halt stop ast va LEDred roshan
                 PORTA.0=0;PORTA.1=1; PORTC.0=1;
                                         }
                                                                                     ...
                                                                                     //motor samte bala   harekat mikonad
          if(bala==0 && play==1 && stepp>0 ){
         for(i=0;i<=7;i++)
          {
           PORTC=step[i];
           delay_ms(100);
            ...
            if(i==7)
               {
                      ertefa++;
                                                                                ...
                                                                                //motor samte payin  harekat mikonad
         if(bala==1 && ertefa!=0){
         for(i=7;i>=0;i--)
          {
           PORTC=step[i];
           delay_ms(100);
            ...
            if(PIND.1==1)                  //check kardan dokme stop
             ...
             if(stepp==0){                   //check kardan tedad  bala payin raftan pa
                   bala=1;
                        play=0;
                         PORTA.0=0;PORTA.1=1;
                         }
